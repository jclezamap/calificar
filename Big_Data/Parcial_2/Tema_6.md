...
**2. Partición de Datos:**   
   ...
   b) Es OBLIGATORIO usar la semilla aleatoria `random_state=650`.   
...
**4. Árbol de Decisión (Optimización):**   
   a) Use `GridSearchCV` con el modelo `DecisionTreeClassifier(random_state=650)`.   
...
**7. k-NN y Permutación:**   
   ...
   b) Calcule la importancia por permutación usando `permutation_importance` con `random_state=650` sobre el conjunto de prueba.   
